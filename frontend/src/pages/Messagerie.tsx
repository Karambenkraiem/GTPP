...
import { MessageSquare, Pin, Trash2, Send, AlertTriangle, Info, ClipboardList } from 'lucide-react';
...
// French month/day labels without locale dependency
...
function timeLabel(iso: string): string {
  ...
  return `${d.getDate()} ${MOIS[d.getMonth()]} ${hhmm}`;
}

type TypeConfig = { label: string; pill: string; border: string; icon: React.ElementType };

const TYPE_CONFIG: Record<TypeMessage, TypeConfig> = {
  consigne: {
    label: 'Consigne',
...
export default function Messagerie() {
  ...
  // Auto-scroll and mark read when new messages arrive
  useEffect(() => {
    if (messages.length > prevLenRef.current) {
      const behavior = prevLenRef.current === 0 ? 'auto' : 'smooth';
  ...
  // Group regular messages by calendar day
  const dayGroups: { day: string; label: string; items: Message[] }[] = [];
  regularMessages.forEach(msg => {
    const key = new Date(msg.cree_le).toDateString();
    const grp = dayGroups.find(g => g.day === key);
    if (grp) grp.items.push(msg);
    else dayGroups.push({ day: key, label: dayLabel(msg.cree_le), items: [msg] });
  });

  const renderMessage = (msg: Message) => {
    const cfg = TYPE_CONFIG[msg.type] ?? TYPE_CONFIG.information;
        ...
        <div className={`min-w-0 max-w-[80%] flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
          ...
          <div className={`flex items-center gap-1 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity ${isOwn ? 'flex-row-reverse' : ''}`}>
            {canPin && (
              <button
                ...
                className={`text-[11px] flex items-center gap-0.5 px-1.5 py-0.5 rounded transition-colors ${
                  msg.is_epingle ? 'text-amber-400 hover:text-amber-300' : 'text-slate-500 hover:text-amber-400'
                }`}
              >
                <Pin size={10} />
            ...
            {canDeleteMsg(msg.auteur_id) && (
              <button
                onClick={() => { if (window.confirm('Supprimer ce message ?')) deleteMutation.mutate(msg.id); }}
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="sticky top-0 z-10 flex-shrink-0 bg-slate-900 border-b border-slate-700 px-4 py-3 flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-slate-400 text-xs">
            {messages.length} message{messages.length !== 1 ? 's' : ''} · actualisation auto. toutes les 10s
          </p>
        </div>
      </div>
      ...
      {/* ── Zone messages ── */}
      <div ref={chatRef} className="flex-1 px-4 py-4 pb-44">
        {isLoading && (
        ...
        {isError && (
          <div className="flex flex-col items-center justify-center py-20 text-red-400">
            ...
            <p className="text-xs mt-1 text-slate-500">Vérifiez votre connexion et rechargez la page</p>
          </div>
        )}

        {!isLoading && !isError && regularMessages.length === 0 && (
